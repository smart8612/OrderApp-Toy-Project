...
extension SceneDelegate {
    ...
    func scene(_ scene: UIScene, restoreInteractionStateWith stateRestorationActivity: NSUserActivity) {
        update(by: stateRestorationActivity.order)
        update(by: StateRestorationController(userActivity: stateRestorationActivity))
    }
    
    private func update(by restoredOrder: Order?) {
        guard let restoredOrder = restoredOrder else { return }
        RestaurantController.shared.restore(order: restoredOrder)
    }
    
    private func update(by uiState: StateRestorationController?) {
        guard let uiState = uiState else { return }
        sceneHirarchyController.restore(state: uiState)
    }
